...
type
   ...
   { Facade for unified layer access }
   TLayerFacade = class
   private
      fInputLayer: ^TLayer;
      fHiddenLayers: ^array of TLayer;
      fOutputLayer: ^TLayer;
      fLayerSizes: array of Integer;
      fLayerOffsets: array of Integer;
      fTotalNeurons: Integer;
      procedure CalculateOffsets;
      procedure GetLayerAndIndex(NeuronIndex: Integer; out LayerType: Integer; out LocalIndex: Integer);
   public
      constructor Create(var InputLayer: TLayer; var HiddenLayers: array of TLayer; var OutputLayer: TLayer);
      function GetNeuronOutput(NeuronIndex: Integer): Double;
      function GetNeuronError(NeuronIndex: Integer): Double;
      function GetNeuronBias(NeuronIndex: Integer): Double;
      function GetNeuronWeight(NeuronIndex, WeightIndex: Integer): Double;
      function GetNumWeights(NeuronIndex: Integer): Integer;
      procedure SetNeuronOutput(NeuronIndex: Integer; Value: Double);
      procedure SetNeuronError(NeuronIndex: Integer; Value: Double);
      procedure SetNeuronBias(NeuronIndex: Integer; Value: Double);
      procedure SetNeuronWeight(NeuronIndex, WeightIndex: Integer; Value: Double);
      function GetTotalNeurons: Integer;
      function GetLayerStartIndex(LayerIndex: Integer): Integer;
...
constructor TLayerFacade.Create(var InputLayer: TLayer; var HiddenLayers: array of TLayer; var OutputLayer: TLayer);
var
   i: Integer;
begin
   fInputLayer := @InputLayer;
   fHiddenLayers := @HiddenLayers;
   fOutputLayer := @OutputLayer;
   
   // Calculate layer sizes
   SetLength(fLayerSizes, Length(HiddenLayers) + 2); // Input + Hidden + Output
   fLayerSizes[0] := Length(InputLayer.Neurons);
   for i := 0 to High(HiddenLayers) do
      fLayerSizes[i + 1] := Length(HiddenLayers[i].Neurons);
   fLayerSizes[High(fLayerSizes)] := Length(OutputLayer.Neurons);
   
   CalculateOffsets;
end;

procedure TLayerFacade. CalculateOffsets;
var
   i: Integer;
begin
   SetLength(fLayerOffsets, Length(fLayerSizes));
   fLayerOffsets[0] := 0;
   fTotalNeurons := fLayerSizes[0];
   
   for i := 1 to High(fLayerSizes) do
   begin
      fLayerOffsets[i] := fLayerOffsets[i - 1] + fLayerSizes[i - 1];
      fTotalNeurons := fTotalNeurons + fLayerSizes[i];
   end;
end;

procedure TLayerFacade. GetLayerAndIndex(NeuronIndex: Integer; out LayerType: Integer; out LocalIndex: Integer);
var
   i: Integer;
begin
   LayerType := -1;
   LocalIndex := NeuronIndex;
   
   for i := 0 to High(fLayerOffsets) do
   begin
      if LocalIndex < fLayerSizes[i] then
      begin
         LayerType := i;
         Exit;
      end;
      LocalIndex := LocalIndex - fLayerSizes[i];
   end;
end;

function TLayerFacade.GetNeuronOutput(NeuronIndex: Integer): Double;
var
   LayerType, LocalIndex: Integer;
begin
   GetLayerAndIndex(NeuronIndex, LayerType, LocalIndex);
   
   if LayerType = 0 then
      Result := fInputLayer^. Neurons[LocalIndex].Output
   else if LayerType = High(fLayerSizes) then
      Result := fOutputLayer^.Neurons[LocalIndex].Output
   else
      Result := fHiddenLayers^[LayerType - 1].Neurons[LocalIndex].Output;
end;

function TLayerFacade.GetNeuronError(NeuronIndex: Integer): Double;
var
   LayerType, LocalIndex: Integer;
begin
   GetLayerAndIndex(NeuronIndex, LayerType, LocalIndex);
   ...
   if LayerType = 0 then
      Result := fInputLayer^. Neurons[LocalIndex].Error
   else if LayerType = High(fLayerSizes) then
      Result := fOutputLayer^. Neurons[LocalIndex].Error
   else
      Result := fHiddenLayers^[LayerType - 1].Neurons[LocalIndex].Error;
end;

function TLayerFacade.GetNeuronBias(NeuronIndex: Integer): Double;
var
   LayerType, LocalIndex: Integer;
begin
   GetLayerAndIndex(NeuronIndex, LayerType, LocalIndex);
   ...
   if LayerType = 0 then
      Result := fInputLayer^.Neurons[LocalIndex]. Bias
   else if LayerType = High(fLayerSizes) then
      Result := fOutputLayer^.Neurons[LocalIndex].Bias
   else
      Result := fHiddenLayers^[LayerType - 1].Neurons[LocalIndex]. Bias;
end;

function TLayerFacade.GetNeuronWeight(NeuronIndex, WeightIndex: Integer): Double;
var
   LayerType, LocalIndex: Integer;
begin
   GetLayerAndIndex(NeuronIndex, LayerType, LocalIndex);
   ...
   if LayerType = 0 then
      Result := fInputLayer^.Neurons[LocalIndex].Weights[WeightIndex]
   else if LayerType = High(fLayerSizes) then
      Result := fOutputLayer^.Neurons[LocalIndex]. Weights[WeightIndex]
   else
      Result := fHiddenLayers^[LayerType - 1].Neurons[LocalIndex].Weights[WeightIndex];
end;

function TLayerFacade.GetNumWeights(NeuronIndex: Integer): Integer;
var
   LayerType, LocalIndex: Integer;
begin
   GetLayerAndIndex(NeuronIndex, LayerType, LocalIndex);
   ...
   if LayerType = 0 then
      Result := Length(fInputLayer^.Neurons[LocalIndex].Weights)
   else if LayerType = High(fLayerSizes) then
      Result := Length(fOutputLayer^.Neurons[LocalIndex].Weights)
   else
      Result := Length(fHiddenLayers^[LayerType - 1].Neurons[LocalIndex].Weights);
end;

procedure TLayerFacade.SetNeuronOutput(NeuronIndex: Integer; Value: Double);
var
   LayerType, LocalIndex: Integer;
begin
   GetLayerAndIndex(NeuronIndex, LayerType, LocalIndex);
   ...
   if LayerType = 0 then
      fInputLayer^.Neurons[LocalIndex].Output := Value
   else if LayerType = High(fLayerSizes) then
      fOutputLayer^.Neurons[LocalIndex]. Output := Value
   else
      fHiddenLayers^[LayerType - 1]. Neurons[LocalIndex].Output := Value;
end;

procedure TLayerFacade.SetNeuronError(NeuronIndex: Integer; Value: Double);
var
   LayerType, LocalIndex: Integer;
begin
   GetLayerAndIndex(NeuronIndex, LayerType, LocalIndex);
   ...
   if LayerType = 0 then
      fInputLayer^.Neurons[LocalIndex].Error := Value
   else if LayerType = High(fLayerSizes) then
      fOutputLayer^.Neurons[LocalIndex].Error := Value
   else
      fHiddenLayers^[LayerType - 1].Neurons[LocalIndex].Error := Value;
end;

procedure TLayerFacade.SetNeuronBias(NeuronIndex: Integer; Value: Double);
var
   LayerType, LocalIndex: Integer;
begin
   GetLayerAndIndex(NeuronIndex, LayerType, LocalIndex);
   ...
   if LayerType = 0 then
      fInputLayer^.Neurons[LocalIndex]. Bias := Value
   else if LayerType = High(fLayerSizes) then
      fOutputLayer^.Neurons[LocalIndex].Bias := Value
   else
      fHiddenLayers^[LayerType - 1].Neurons[LocalIndex]. Bias := Value;
end;

procedure TLayerFacade.SetNeuronWeight(NeuronIndex, WeightIndex: Integer; Value: Double);
var
   LayerType, LocalIndex: Integer;
begin
   GetLayerAndIndex(NeuronIndex, LayerType, LocalIndex);
   ...
   if LayerType = 0 then
      fInputLayer^.Neurons[LocalIndex].Weights[WeightIndex] := Value
   else if LayerType = High(fLayerSizes) then
      fOutputLayer^.Neurons[LocalIndex].Weights[WeightIndex] := Value
   else
      fHiddenLayers^[LayerType - 1]. Neurons[LocalIndex].Weights[WeightIndex] := Value;
end;

function TLayerFacade.GetTotalNeurons: Integer;
begin
   Result := fTotalNeurons;
end;

function TLayerFacade.GetLayerStartIndex(LayerIndex: Integer): Integer;
begin
   if (LayerIndex >= 0) and (LayerIndex <= High(fLayerOffsets)) then
      Result := fLayerOffsets[LayerIndex]
   else
      Result := -1;
end;
...
begin
   if (LayerIndex >= 0) and (LayerIndex <= High(fLayerSizes)) then
      Result := fLayerSizes[LayerIndex]
   else
      Result := 0;
end;
...
begin
   ...
   // Create facade after all layers are initialized
   LayerFacade := TLayerFacade.Create(InputLayer, HiddenLayers, OutputLayer);
end;
